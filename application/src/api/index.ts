...
export const api = {
    ...
    async get(searchTerm:string, page:number){
        const response = await  axios.get(`${baseURL}/tag?label=${searchTerm}&page=${page ? page : 1}&limit=30`)
        return response.data
    },
    ...
    async put(tag:{id:string, label:string}) {
        const response = await axios.put(`${baseURL}/tag/${tag.id}`, {label:tag.label} )
        return response
    },
